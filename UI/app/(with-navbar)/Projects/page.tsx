...
export default function ProjectPage() {
  ...
  const deleteTask = (id: number) => {
      ...
      setTasks2((prev) =>
        prev.map((t) =>
          t.id === id
            ? {
                ...t,
                status: t.status === "complete" ? "in-progress" : "complete",
              }
            : t,
        ),
      );
    ...
    const deleteTask2 = (id: number) => {
      setTasks2((prev) => prev.filter((t) => t.id !== id));
    };
      ...
      <main className="ml-[50px] mt-[56px] p-6 min-h-[calc(100vh-56px)]  text-[var(--text-primary)]">
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 space-y-6 w-[60vw]">
            ...
            <div className="rounded-xl p-6 shadow border bg-[var(--card-bg)] border-[var(--border-color)] shadow-[var(--card-shadow)]">
              <div className="flex justify-between items-center border-b pb-3 mb-4 border-[var(--border-color)]">
                <h2 className="text-lg font-bold text-[var(--text-primary)]">
                  Current Tasks
                </h2>

                <button className="text-white px-3 py-1 rounded text-xs bg-[var(--accent-color)] hover:bg-[var(--accent-hover)] cursor-pointer transition-all">
                  + Add Task
                </button>
              </div>
              ...
              <div className="space-y-3">
                {tasks2.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-center gap-3 p-3 rounded-lg border-l-4 bg-[var(--bg-tertiary)] border-[var(--border-color)] "
                    style={{
                      borderLeftColor:
                  ...
                  >
                        ...
                        {task.assignee && (
                          <>
                            ...
                            <div className="flex items-center gap-1">
                              <div className="w-6 h-6 rounded-full text-white text-xs flex items-center justify-center font-semibold bg-[var(--accent-color)]">
                                {task.initials}
                              </div>
                    ...
                    {/* Delete Button */}
                    <button
                      onClick={() => deleteTask2(task.id)}
                      className="px-2 py-1 text-xs rounded text-white hover:bg-[var(--delete-hover)] cursor-pointer transition-all"
                    >
                      🗑
                    </button>
                  </div>
    ...
    text-[var(--text-primary)] border-[var(--border-color)]"
        ...
        bg-gradient-to-br from-[var(--accent-color)] to-[#0099ff]"
                  ...
                  <div className="absolute bottom-2 right-2 grid gap-2">
                    <button className="text-[11px] font-semibold px-2 py-0.5 rounded transition-all bg-[var(--accent-color)] text-white hover:bg-[var(--accent-hover)] cursor-pointer">
                      View
                    </button>

                    <button className="text-[11px] font-semibold px-2 py-0.5 rounded transition-all bg-[#d32f2f] text-white hover:bg-[#b71c1c] cursor-pointer">
                      Reject
                    </button>
                  </div>
                </div>
         ...
         bg-gradient-to-br from-[var(--accent-color)] to-[#0099ff]"
                  ...
                  <div className="absolute bottom-2 right-2 grid gap-2">
                    <button className="text-[11px] font-semibold px-2 py-1 rounded transition-all bg-[var(--accent-color)] text-white">
                      View
                    </button>

                    <button className="text-[11px] font-semibold px-2 py-1 rounded transition-all bg-[#d32f2f] text-white">
                      Reject
                    </button>
                  </div>
                </div>
